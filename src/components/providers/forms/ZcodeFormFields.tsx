...
interface ZcodeFormFieldsProps {
  ...
  kind: ZcodeKind;
  onKindChange: (kind: ZcodeKind) => void;
  models: ZcodeModel[];
  onModelsChange: (models: ZcodeModel[]) => void;
}

type BaseUrlErrorCode = "empty" | "invalid" | "scheme";

const BASE_URL_ERROR_I18N_KEY: Record<BaseUrlErrorCode, string> = {
  empty: "zcode.form.baseUrlRequired",
  scheme: "zcode.form.baseUrlScheme",
  invalid: "zcode.form.baseUrlInvalid",
};
    ...
    <>
      <div className="space-y-2">
        <FormLabel htmlFor="zcode-kind">
          {t("zcode.form.kind", { defaultValue: "API 类型" })}
        </FormLabel>
        <Select
          value={kind}
          onValueChange={(v) => onKindChange(v as ZcodeKind)}
        >
      ...
      <div className="space-y-2">
        <FormLabel htmlFor="zcode-baseurl">
          {t("zcode.form.baseUrl", { defaultValue: "API 端点" })}
        </FormLabel>
        <Input
          id="zcode-baseurl"
          value={baseUrl}
      ...
      <ApiKeySection
        ...
        // ZCode 没有免 key 的官方供应商：所有预设都需用户自填 key，
        // 故不让 official 禁用输入框（与 Hermes/DSH 对齐）。
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <FormLabel>
            {t("zcode.form.models", { defaultValue: "模型列表" })}
          </FormLabel>
          <div className="flex gap-1">
            ...
            >
              ...
              {t("zcode.form.addModel", { defaultValue: "添加模型" })}
            </Button>
          </div>
        </div>
            ...
            {models.map((model, index) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <div className="flex-1 space-y-1">
                    <label className="text-xs text-muted-foreground">
                      {t("zcode.form.modelId", { defaultValue: "模型 ID" })}
                    </label>
                    <div className="flex gap-1">
                      <Input
                        value={model.id}
                        ...
                        placeholder={t("zcode.form.modelIdPlaceholder", {
                ...
                <div className="space-y-1">
                  <label className="text-xs text-muted-foreground">
                    {t("zcode.form.modelName", { defaultValue: "模型名称" })}
                  </label>
                  <Input
                    value={model.name ?? ""}
                    onChange={(e) =>
                    ...
                    placeholder={t("zcode.form.modelNamePlaceholder", {
                      defaultValue: "GLM-5.1",
                    })}
                  />
                </div>
        ...
        <p className="text-xs text-muted-foreground">
          {t("zcode.form.modelsHint", {
            defaultValue:
              "该供应商可用的模型列表。模型的可选属性（如 reasoning、limit）可在下方 JSON 中编辑。",
          })}
        </p>
      </div>

      <p className="text-xs text-muted-foreground">
        {t("zcode.form.runtimeNote", {
          defaultValue: "供应商配置写入后需重启 ZCode 才会生效。",
        })}
      </p>
    </>
  );
