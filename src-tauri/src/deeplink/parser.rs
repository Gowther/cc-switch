...
fn parse_provider_deeplink(
    ...
    if !matches!(
        app.as_str(),
        "claude" | "codex" | "gemini" | "opencode" | "openclaw" | "hermes" | "dsh" | "zcode"
    ) {
        return Err(AppError::InvalidInput(format!(
            "Invalid app type: must be 'claude', 'codex', 'gemini', 'opencode', 'openclaw', 'hermes', 'dsh', or 'zcode', got '{app}'"
        )));
    }
...
fn parse_mcp_deeplink(
    ...
    for app in apps.split(',') {
        ...
        if !matches!(
            ...
            return Err(AppError::InvalidInput(format!(
                "Invalid app in 'apps': must be 'claude', 'codex', 'gemini', 'opencode', 'openclaw', 'hermes', 'dsh', or 'zcode', got '{trimmed}'"
            )));
        }
    }
