...
### Claude Code、Claude Desktop、Codex、Gemini CLI、OpenCode、OpenClaw、Hermes Agent、DeepSeek Harness (dsh)、ZCode のオールインワン管理ツール
...
最新の AI コーディングは Claude Code、Claude Desktop、Codex、Gemini CLI、OpenCode、OpenClaw、Hermes、DeepSeek Harness (dsh)、ZCode などのツールに依存していますが、各ツールの設定形式はバラバラです。API プロバイダを切り替えるたびに JSON、TOML、`.env` ファイルを手動で編集する必要があり、複数ツール間で MCP や Skills を統一的に管理する手段もありません。
...
- **1 つのアプリで 9 つのツール** -- Claude Code、Claude Desktop、Codex、Gemini CLI、OpenCode、OpenClaw、Hermes、DeepSeek Harness (dsh)、ZCode を単一インターフェースで管理
- **手動編集は不要** -- AWS Bedrock、NVIDIA NIM、コミュニティリレーなど 50 以上のプロバイダプリセットを内蔵。選んで切り替えるだけ
- **統一 MCP・Skills 管理** -- 1 つのパネルで Claude、Codex、Gemini、OpenCode、Hermes、dsh、ZCode の MCP サーバーと Skills を双方向同期で管理
...
- **9 つの対応ツール、50 以上のプリセット** -- Claude Code、Claude Desktop、Codex、Gemini CLI、OpenCode、OpenClaw、Hermes、dsh (DeepSeek Harness)、ZCode。キーをコピーしてワンクリックでインポート
...
- **統一 MCP パネル** -- Claude、Codex、Gemini、OpenCode、Hermes、dsh、ZCode の MCP サーバーを管理、双方向同期、Deep Link インポート対応
...
CC Switch は **Claude Code**、**Claude Desktop**、**Codex**、**Gemini CLI**、**OpenCode**、**OpenClaw**、**Hermes**、**dsh (DeepSeek Harness)**、**ZCode** の 9 つのツールに対応しています。各ツールに専用のプロバイダプリセットと設定管理が用意されています。
